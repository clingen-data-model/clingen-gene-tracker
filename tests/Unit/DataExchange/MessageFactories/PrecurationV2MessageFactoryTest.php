 ...
 */
class PrecurationV2MessageFactoryTest extends TestCase
{
    public function setup():void
    {
        ...
        $this->factory = new PrecurationV2MessageFactory();
    }
    ...
    public function makesUpdatedMessage()
    {
        ...
        $included_phenotypes = factory(Phenotype::class, 1)->create();
        $included_phenotypes->each(function ($ph) use ($gene) {
            $gene->addPhenotype($ph);
        });
        $excluded_phenotypes = factory(Phenotype::class, 3)->create()->sortBy('mim_number')->values();
        $excluded_phenotypes->each(function ($ph) use ($gene) {
            $gene->addPhenotype($ph);
        });
        ...
        $included_phenotypes->each(function ($ph) {
            $this->curation->addPhenotype($ph);
        });
        ...
        $data = array_merge($this->getBaseData(), [
            ...
            'omim_phenotypes' => [
                'included' => $included_phenotypes->pluck('mim_number')->toArray(),
                'excluded' => $excluded_phenotypes->pluck('mim_number')->toArray(),
            ],
            'labelled_omim_phenotypes' => [
                'included' => $included_phenotypes->map(function ($ph) {
                    return ['mim_number' => $ph->mim_number, 'label' => $ph->name];
                })->toArray(),
                'excluded' => $excluded_phenotypes->map(function ($ph) {
                    return ['mim_number' => $ph->mim_number, 'label' => $ph->name];
                })->toArray(),
            ]
        ]);
    ...
    private function assertMessageEquals($eventType, $data, $message)
    {
        ...
        $this->assertEquals(2, $message['schema_version']);
