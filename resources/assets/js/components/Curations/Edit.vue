                ...
                <div class="row">
                    <div class="col-md-2 border-right">
                        <nav class="nav flex-column">
                            <router-link v-for="(step, idx) in steps" :key="idx" :to="$route.path + '#' + idx"
                                class="nav-link" :class="{ active: (currentStep == idx) }">
                                {{ step.title }}
                            </router-link>
                        </nav>
                    </div>
                    <div class="col-md-10">
                        <component :is="currentStep" :value="updatedCuration" :errors="errors"
                            @input="updatedCuration = $event" ref="editPage">
                        </component>
                    </div>
                </div>
                <hr>
                <div class="row">
                    <div class="col-md-4">
                        ...
                        <button type="button" class="btn btn-secondary"
                            @click="$router.push('/curations')">Cancel</button>
                    </div>
                    <div class="col-md-8 text-right">
                        <button type="button" class="btn btn-secondary" id="curation"
                            @click="updateCuration()">Save</button>
                        <button v-if="nextStep" type="button" class="btn btn-secondary"
                            @click="updateCuration(exit)">Save &amp;
                            exit</button>
                        <button variant="primary" @click="updateCuration(navBack, 'back')"
                            v-show="currentStepIdx > 0">Back</button>
                        <button variant="primary" @click="updateCuration(navNext, 'next')">
