...
class PrecurationV2MessageFactory implements MessageFactoryInterface
{
    ...
    private function assembleData($curation)
    {
        ...
        return array_filter([
            ...
            'labelled_omim_phenotypes' => $this->getLabeledOmimPhenotypes($curation),
            // Keep using notes instead of renamed 'curation_notes' because that is what consumers expect
    ...
    private function getLabeledOmimPhenotypes($curation)
    {
        if ($curation->phenotypes->count() === 0) {
            return null;
        }
        return [
            "included" => $curation->phenotypes->map(function($phenotype) {
                return [
                    "mim_number" => $phenotype->mim_number,
                    "label" => $phenotype->name
                ];
            })->toArray(),
            "excluded" => $curation->excludedPhenotypes->map(function($phenotype) {
                return [
                    "mim_number" => $phenotype->mim_number,
                    "label" => $phenotype->name
                ];
